//! Key-length guard: an over-limit key is rejected identically on both backends.
//!
//! Fjall caps keys at 65_536 bytes; RocksDB has no limit. Without the guard a
//! huge doc id (or a long indexed metadata value) would be accepted by RocksDB
//! and refused by Fjall — a silent divergence between the two releases. The guard
//! closes it: both reject, up front.
