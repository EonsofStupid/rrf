...
pub async fn serve(flow: Arc<ReasonReadyObject>, opts: ServeOptions) -> Result<()> {
    ...
    let _server = match &opts.listen {
        Some(addr) => {
            let (bound, task) = rro_net::tcp::serve(addr.clone(), node.clone()).await?;
    ...
    rro_core::events::emit("serve.stop", serde_json::json!({ "signal": sig }));
    Ok(())
}
...
pub async fn wait_for_shutdown() -> &'static str {
    ...
    rro_core::events::emit("signal.received", serde_json::json!({ "signal": sig }));
    sig
}
