...
use rrf_core::{Condition, Embedding, EstateQuery, Filter, Recall, VectorRecord};
...
async fn flush_and_compact_over_the_wire_with_exact_queries_after() {
    ...
    assert!(err.is_ok());
    let unknown = rrf_net::tcp::request(
        addr,
        &rrf_net::Message::request("t", "mt-node", "no_such_verb", serde_json::json!({})),
    )
    .await
    .unwrap();
