...
async fn metrics_and_probes_over_real_http() {
    ...
    let (addr, _task) = rrf_flow::ops::serve_ops("127.0.0.1:0", estate)
        .await
        .unwrap();
    ...
    assert!(body.contains("# TYPE rrf_docs_total gauge"));
    assert!(body.contains("rrf_docs_total 2"), "{body}");
    assert!(body.contains("rrf_feed_seq 2"));
    assert!(body.contains("rrf_applier_backlog 0"));
    assert!(body.contains("rrf_collection_docs{collection=\"ops\"} 2"));
    assert!(body.contains("rrf_issues_total 0"));
