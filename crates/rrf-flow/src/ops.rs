...
pub async fn serve_ops(
    ...
    let listener = TcpListener::bind(addr)
        .await
        .map_err(|e| RrfError::Net(format!("ops bind: {e}")))?;
    let local = listener
        .local_addr()
        .map_err(|e| RrfError::Net(format!("ops local_addr: {e}")))?;
...
/// snapshot — gauges only, all prefixed `rrf_`.
fn render_metrics(estate: &connxism::Estate, started: std::time::Instant) -> Result<String> {
    ...
    gauge(
        &mut out,
        "rrf_collections_total",
        "Named collections.",
        h.collections.len() as f64,
    );
    for (name, count) in &h.collections {
        out.push_str(&format!(
            "rrf_collection_docs{{collection=\"{name}\"}} {count}\n"
        ));
    }
    gauge(
        &mut out,
