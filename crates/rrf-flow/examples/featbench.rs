...
//! latency for `watch`. Run: `cargo run --release -p rrf-flow --example featbench`

use std::sync::Arc;
use std::time::Instant;

use rrf_core::{Condition, Embedding, EstateQuery, Filter, Recall, SparseVector, VectorRecord};
...
async fn main() {
    ...
    let (addr, _task) = rrf_net::tcp::serve("127.0.0.1:0", Arc::new(node))
        .await
        .unwrap();
    let client = rrf_client::Client::new(addr.to_string());
