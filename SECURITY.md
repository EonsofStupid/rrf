...
**[Report a vulnerability](https://github.com/EonsofStupid/rrobjects/security/advisories/new)**
...
RRO is an **embedded** engine. It runs in your process, on your machine, against your data.
...
- **The `a2a` surface** (`rro-net`) — this is the one place RRO listens. Anything that lets a
  remote peer read, write, or trigger work it shouldn't.
- **Connector credentials** — RRO is not a secret custodian. If a credential ever reaches
...
RRO is pre-release and moves fast. **Only `main` is supported.** There are no backports.
